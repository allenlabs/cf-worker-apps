    ...
    >
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
        <input
          ...
          aria-label={t('nudge.clientIdLabel')}
        ...
        <input
          ...
          placeholder={t('nudge.friendlyName')}
          aria-label={t('nudge.nameLabel')}
      ...
      <div className="flex justify-end">
        ...
        >
          {busy ? t('nudge.issuing') : t('nudge.issueToken')}
        </button>
      </div>
    </form>
  );
...
function AdminPage() {
  ...
  async function handleDelete(clientId: string) {
    if (!confirm(t('nudge.revokeConfirm', { clientId }))) return;
    setBusy(true);
    try {
  ...
  return (
    <>
      ...
      <div className="max-w-3xl mx-auto p-4">
        <h1 className="text-lg font-semibold text-slate-200 mb-1">{t('nudge.apiClients')}</h1>
        <p className="text-xs text-slate-500 mb-4">{t('nudge.apiClientsSubtitle')}</p>
        {issued ? (
          <div className="mb-4">
            <IssuedSecret
        ...
        {clients.length === 0 ? (
          <p className="text-sm text-slate-400" data-testid="no-clients">{t('nudge.noTokens')}</p>
        ) : (
