...
export const Route = createFileRoute('/trash')({
  ...
  loader: async ({ deps }) => {
    ...
    if (!workspaceId) {
      return { workspaces, workspaceId: '', pages: [] as PageNode[], trash: [] as TrashItem[] };
    }
    const [pages, trash] = await Promise.all([
      getTree({ data: { workspaceId } }),
      pagesTrash({ data: { workspaceId } }),
    ]);
    return { workspaces, workspaceId, pages, trash };
  },
  component: TrashPage,
});

function TrashPage() {
  const { workspaces, workspaceId, pages, trash } = Route.useLoaderData();
  ...
  async function handlePurge(item: TrashItem) {
    ...
    const ok = window.confirm(
      `Permanently delete "${item.title || 'Untitled'}" and everything inside it? This cannot be undone.`,
    );
    if (!ok) return;
    setBusy(true);
    try {
    ...
    <div className="flex h-[calc(100vh-3rem)] -mx-4 -my-6">
      ...
      <div className="flex-1 overflow-y-auto p-8">
        <div className="max-w-2xl mx-auto">
          <h1 className="text-xl font-semibold mb-1">Trash</h1>
