...
// Per-request drizzle instance cache. WeakMap keyed on the in-flight Request, so
// every `makeDb()` call inside a single request reuses one client (one pool);
// the entry drops as soon as the request handler exits.
type DrizzleDb = ReturnType<typeof drizzle<typeof schema>>;
const dbByRequest = new WeakMap<Request, DrizzleDb>();
...
/** Build (or reuse, per request) the application DB from a connection string. */
function cachedForRequest(connectionString: string): DrizzleDb {
...
/** The application DB (env.HYPERDRIVE), per-request cached. */
export function makeDb(env: { HYPERDRIVE: Hyperdrive }): DrizzleDb {
  return cachedForRequest(env.HYPERDRIVE.connectionString);
}
